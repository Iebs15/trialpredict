...
const AutocompleteInput = ({
  ...
  return (
    <div className="relative">
      <label
        htmlFor={id}
        className="block text-sm font-semibold text-slate-700 mb-3 tracking-wide uppercase"
      >
        {label}
      </label>
      <div className="relative">
        ...
        <input
          ...
          onBlur={handleBlur}
          onFocus={() =>
            value.length > 0 && setIsOpen(filteredSuggestions.length > 0)
          }
          autoComplete="off"
        />
...
export default function BiomarkerDiseaseSearch() {
  ...
  const handleBiomarkerSearch = async () => {
    ...
    try {
      const response = await fetch(
        ...
        }/biomarker_landscape?targetName=${encodeURIComponent(
          biomarkerInput.trim()
        )}`
      );
  ...
  const handleDiseaseSearch = async () => {
    ...
    try {
      const response = await fetch(
        `${
          import.meta.env.VITE_API_URL
        }/disease_landscape?disease=${encodeURIComponent(diseaseInput.trim())}`
      );
  ...
  const handleSymptomSearch = async () => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_URL}/symptom?name=${encodeURIComponent(
          symptomInput.trim()
        )}`
      );
  ...
  const handleTabClick = (tabId) => {
    setActiveTab(tabId);
    if (tabId === "biomarker") {
      navigate("/treatment-comparison"); // Navigate to /treatment-comparison when the biomarker tab is clicked
    }
  };
  return (
    <div className="w-full max-w-4xl bg-white rounded-2xl shadow-2xl border border-slate-200 overflow-visible">
      ...
      <div className="bg-gradient-to-r from-slate-50 to-blue-50 px-8 py-6 border-b border-slate-200">
        ...
        <p className="text-slate-600 font-medium">
          Advanced biomarker-disease relationship analysis
        </p>
      </div>
          ...
          {tabs.map((tab) => {
            ...
            return (
              <button
                ...
                onClick={() => handleTabClick(tab.id)} // Use handleTabClick for all tabs
              >
