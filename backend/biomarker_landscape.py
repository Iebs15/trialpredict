...
# --- 2. Map Target Name to Target ID ---
...
def interactive_plot_graph(G, output_html="knowledge_graph.html", max_nodes=200):
    ...
    # Limit number of nodes if too large
    ...
    for node, data in subG.nodes(data=True):
        ...
        net.add_node(str(node), label=label, color=color_map.get(ntype, 'gray'))

    for source, target, data in subG.edges(data=True):
        net.add_edge(str(source), str(target), title=data['type'])
